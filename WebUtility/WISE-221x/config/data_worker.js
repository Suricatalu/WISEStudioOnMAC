...
var raw2FilterSlotAndIOType = function(rawData, slot){
    ...
    for (var i = 0; i < len; ++i) {
    	if(slotArray[i] != undefined){
    		//var pe = slotArray[i][_DATA_FORMAT_PE];//no pe in sub-1G
		    ...
		    if(logTypeStatistic[io] == undefined){
		    	logTypeStatistic[io] = 1;
		    }
		    else{
		    	logTypeStatistic[io]++;
		    }
		    /*if(defaultIOType == io){
			    ...
			    if(defaultIOType != 11 & defaultIOType!= 17 & defaultIOType!= 0){
...
var json2Array = function(jsonObj){
	...
	var logDataToObj = function(rssi, tim, uid, mac, data) {
	    return {
	    	//pe: pe,//Sub1G no PE
	    	rssi: rssi,//new in Sub1G
	        tim: tim,
	        uid: uid,
	        mac: mac,
	...
	var logTypeStatistic = {};
	var logTrendDatas = {};
	var logMsg = jsonObj;//jsonObj["LogMsg"];
	//var logMsgLen = logMsg.length;//only one for Sub1G

    //for (var i = 0; i < logMsgLen; ++i) {//only one for Sub1G
        //var pe = logMsg[i].PE;
        //var pe = 128; //Sub1G no PE, set 128(periodic) as default
        var rssi = logMsg.Rssi || "****"; //new in Sub1G
        ...
        for (var j = 0; j < recordArray.length; ++j) {
        	var logObj = logDataToObj(rssi, tim, uid, mac, recordArray[j]);
        	if(slotStatatistic[logObj["slot"]] == undefined){
        		slotStatatistic[logObj["slot"]] = 1;
        	}
        	var data = [logObj["rssi"],logObj["tim"],logObj["uid"],logObj["mac"],logObj["slot"],logObj["channel"],logObj["ioType"], logObj["value"]];
