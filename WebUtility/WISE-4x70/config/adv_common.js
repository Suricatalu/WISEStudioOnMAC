...
$.extend(true, Advantech, {
    Profile: {
        ...
        CloudErrorCodeEmun: {
            ...
            10:  "Other HTTP error code",
            11:  "HTTP 400 Bad request",
            12:  "HTTP 401 Unauthorized",
            13:  "HTTP 403 Forbidden",
            14:  "HTTP 404 Not found",
            15:  "HTTP 409 Conflict",
            16:  "HTTP 429 Too many request",
            17:  "HTTP 500 Internal error",
            18:  "HTTP 503 Service unavailable",
            19:  "HTTP 507 Insufficient storage",
            //DropBox
...
var WiseModule = {
    ioBoard:{
        ...
        "S617":{
            description: "1-port RS-485 and 2-ch Analog Input and DIO",
            ao_Ch: 0,
            ai_Ch: 2,
            di_Ch: 2,
            do_Ch: 1,
    ...
    baseBoard:{
        "WISE-4470":{
            ...
            isSupportWorkingMode: false,
            isSupportRegisteredServiceDomain: false,
            fwVersion: 'A1.00 B05'
        },
        "WISE-4670":{
            description: "Outdoor 4G WSN",
            hasUSB:false,
            hasSD:false,
            isCoupler: false,
            isSupportP2P: false,
                ...
                var updateProgressForm = function(progress, FileSize /*0~100*/ ) {
                    ...
                    (function loop(){
                        ...
                        if(barVal < currProgress && progress < 100){ //If progress is 0~99, update progress bar
                            ...
                            TimerID = setTimeout(loop, 900);    //Total upload time via VCOM is about 90 sec
...
/*                        if(e.channel == null)
            ...
            DIConfigController: function(baseForm, overviewTable, module, profile) {
                            ...
                            if(Advantech.Data.ModuleData.getInstance().getIoMode() == Advantech.Profile.IoBoardWorkingModeValue.LOW_POWER_MODE &&
                                ...
                                for(var i=0; i< DICfg.length; i++){
                                    if(parseInt(DICfg[i].FtLo)*10 != DICfg[i].FtLo*10 || DICfg[i].FtLo < 1 || DICfg[i].FtLo > 6) {
                                        Advantech.Form.MessageForm.getInstance().showMessageBox("Error", "<i class='fa fa-fw fa-exclamation-triangle'></i>Min. Low Signal Width must be intetger and between 1~6" , null);
                                        return;
                                    }
                                    if(parseInt(DICfg[i].FtHi)*10 != DICfg[i].FtHi*10 || DICfg[i].FtHi < 1 || DICfg[i].FtHi > 6) {
                                        Advantech.Form.MessageForm.getInstance().showMessageBox("Error", "<i class='fa fa-fw fa-exclamation-triangle'></i>Min. High Signal Width must be intetger and between 1~6" , null);
                                        return;
                                    }
                ...
                var refreshPuginForm = function(md) {
                    ...
                    var t = _Self.getBaseForm();
                    var test = _Self.getBaseForm().getPlugInContainer();
                    mPlugInForm.initialForm(_Self.getBaseForm().getPlugInContainer().id);
                    _Self.updateUI();
                };
            },
            ...
            DIPlugInConfigForm: function(formId) {
                ...
                this.initialForm = function(containerName) {
                    ...
                    configForm.innerHTML =
                        ...
                        "<div class='col-lg-8'>" +
                        "<div class='input-group' style='min-width: 20px'>" +
                        //JiaWei 2017/03/22 change DI L/H signal width
                ...
                var setMode = function(containerName, mdArray) {
                    if($.isArray(mdArray)) {
                        ...
                        for (var i = 0; i < mdArray.length; ++i) {
							...
							//Robert(2017/11/09): change again, no DI disable modeArray
            ...
            AIOConfigBaseForm: function(formId) {
                ...
                var eventHandle = function() {
                    ...
                    $("#" + _Self.getFormId() + " #selRng").change(function() {
                        ...
                        if(channel == 0 && (rangeObj.unit == "V" || rangeObj.unit == "mV"))
                    ...
                    $("#" + _Self.getFormId() + " #btnZSCalibration").click(function(){
                        ...
                        rangeObj.min = 0;//always 0 for Zero calibration
                ...
                this.setForm = function(jsonObj) {
                    try{
                        ...
                        if(jsonObj.Ch == 0 && (rangeObj.unit == "V" || rangeObj.unit == "mV"))
                            $("#" + _Self.getFormId() + " .zsCalibration").show();
                        else
                            $("#" + _Self.getFormId() + " .zsCalibration").hide();
                    }catch(e){
...
*/
        ...
        ConfigForm: {
            CloudConfigPanel:function(panelId){
                ...
                this.setOtherPanelId = function(privateServerPanelId, webAccessPanelId, aliyunPanelId, mqttPanelId, chungHwaIotPanelId, azurePanelId){
                    mPrivateServerPanelId = privateServerPanelId;
                    mWebAccessPanelId = webAccessPanelId;
                    mAliyunPanelId = aliyunPanelId;
                ...
                var updateCloudStatusPanel = function(jsonObj){
                    ...
                    if(mSelectedCloudIndex == 1)//dropBox
                    ...
                    if(selCloudIndex != mSelectedCloudIndex || selCloudIndex == Advantech.Profile.CloudServiceVenderEmun.Disabled)
                ...
                var handleCloudServiceSelect = function(selectId){
                    ...
                    $selCloud.change(function(){
                        ...
                        if(index == Advantech.Profile.CloudServiceSelectIndex.PrivateServer){
                        ...
                        }else if(index==Advantech.Profile.CloudServiceSelectIndex.AliyunOSS){
                            _$Panel.find(".cloudPanel").hide();
                            _$Panel.find("#panelAliyun").show();
                ...
                this.setPanel = function(jsonObj) {
                    try{
                        ...
                        if(int(jsonObj.SEn) == 1)
                            $("#"+_Self.getPanelId()+" #waWsEnable").prop("checked", true);
                        else
                            $("#"+_Self.getPanelId()+" #waWsDisable").prop("checked", true);
                        if(int(jsonObj.SSLEn) == 1)
                    ...
                    $("#" + _Self.getPanelId() + " input").each(function() {
                        if(!$(this).is(":disabled") && ($(this).css("display")!='none') && $(this).is(":visible")) {
                            ...
                            if(type === "checkbox") {
                                ...
                                if(this.id === "waSslDisable"){
                                    obj['SSLEn'] = $('#waSslDisable').prop("checked")?0:1;
                                }
                                if(this.id === "waWsDisable"){
                                    obj['SEn'] = $('#waWsDisable').prop("checked")?0:1;
                                }
                            }else{
                                obj[id] = $(this).val();
                            }
            ...
            AliyunOssConfigPanel: function(panelId) {
                var _onAliyunOssCloudSubmitted = null;
                var _refreshAliyunStatus = null;
                var mContextContainerId = panelId;
                var _Self = this;
                this.getPanelId = function() {
                    return mContextContainerId;
                ...
                var handleSubmitted = function(){
                    $("#" + _Self.getPanelId() + " #btnAliyunSubmit").click(function() {
                        if($.isFunction(_Self.onAliyunOssSubmitted())) {
                            _Self.onAliyunOssSubmitted().apply(this, [this, _Self.toJson()]);
                        }
                    });
                };
                this.setPanel = function(jsonObj) {
                    try{
                        ...
                        $("#"+_Self.getPanelId()+" #logEn").val(jsonObj.En);
                ...
                this.refreshAliyunStatus = function(x) {
                    if(!arguments.length) return _refreshAliyunStatus;
                    _refreshAliyunStatus = x;
                };
                this.onAliyunOssSubmitted = function(x) {
                    if(!arguments.length) return _onAliyunOssCloudSubmitted;
                    _onAliyunOssCloudSubmitted = x;
                };
            },
                ...
                this.initialPanel = function() {
                    try {
                        if (module.indexOf('WISE-4670') > -1) {
                            $("#" + _Self.getPanelId() + " .timeSettings").hide();
                        }
...
/*                this.onDataLogSensorOptionSubmitted = function(x) {
                ...
                var addRowToAccessCtrlTable = function(index) {
                    ...
                    for (var i = 0; i < 3; ++i) {
                        ...
                        if(i == 0)
                            ...
                            cell.innerHTML = "<input type='text' id='txtbarAccessCtrlAddress" + index + "' class='form-control' maxlength='15' style='min-width: 150px;'>";
                        }
                    }
                    $("#" + _Self.getTableId() + " tbody").append(row);
            ...
            PeerToPeerModeConfigPanel: function(panelId) {
                this._onPeerToPeerModeSwitch = null;
                ...
                this.initialPage = function(switchContainerId) {
                    ...
                    $("#" + this.getPanelId() + " #btnP2PModeConfig").click(function() {
                        ...
                        var qos = $("#" + _Self.getPanelId() + " #inpP2pComQL").val();
                        var ecs = $("#" + _Self.getPanelId() + " #inpP2pComECS").val();
                        if(md !== 0)
                            $("#" + _Self.getPanelId() + " #p2pCommonConfigGroup").show();
                        else
                            $("#" + _Self.getPanelId() + " #p2pCommonConfigGroup").hide();

                        if($.isFunction(_Self._onPeerToPeerModeSwitch)){
                            _Self._onPeerToPeerModeSwitch(this, {
                                Md: Number(md),
                                QL: Number(qos),
                                ECS: Number(ecs)
                            });
...
/*                      for(var i = 0; i < mObserver.length; ++i){
                ...
                this.toJsonFormat = function(){
                    ...
                    return {Md: Number(md), QL: Number(qos), ECS: Number(ecs)};
                };
                ...
                this.setP2PModePanel = function(jsonObj) {
                    try{
                        ...
                        $("#" + this.getPanelId() + " #inpP2pComQL").val(jsonObj.QL);
                        $("#" + this.getPanelId() + " #inpP2pComECS").val(jsonObj.ECS);
                        if(jsonObj.Md != 0)
                            $("#" + this.getPanelId() + " #p2pCommonConfigGroup").show();
                        else
                            $("#" + this.getPanelId() + " #p2pCommonConfigGroup").hide();
                    }catch(e){
                        throw new Error("PeerToPeerModeConfigPanel: Setting P2P mode parameter failed." +e)
                    }
                };
                ...
                this.onPeerToPeerModeSwitch = function(x) {
                    if(!arguments.length) return this._onPeerToPeerModeSwitch;
                    this._onPeerToPeerModeSwitch = x;
                };
            ...
            P2pConfigPageFactory: function() {
                //this.instanceP2pPage = function(md, slotAmount, pageId, slotIdx, chNum, isAiType) {
                this.instanceP2pPage = function(md, slotAmount, pageId, slotIdx, chNum, channelInfoJson) {
                    if(md === 1) {
                        return new Advantech.Form.AdvancedForm.PeerToPeerBasicMdConfigPage(slotAmount, pageId,
                            slotIdx,
                            chNum,
                            //isAiType);
                            channelInfoJson);
                    }else if(md === 2) {
                        return new Advantech.Form.AdvancedForm.PeerToPeerAdvancedMdConfigPage(slotAmount, pageId,
                            slotIdx,
                            chNum,
                            //isAiType);
                            channelInfoJson);
                    }else{
                        return new Advantech.Form.AdvancedForm.PeerToPeerNotSupportConfigPage(slotAmount, pageId,
                            slotIdx,
                            chNum,
                            //isAiType);
                            channelInfoJson);
            ...
            PeerToPeerMainConfigPanel: function(panelId, p2PModeConfigPanelId, p2PBasicMdPanelId, p2PAdvancedMdPanelId, p2PCommonConfigPageId, profileData) {
                var _onChangedSolt = null;
                var _onConfigSubmitted = null;
                var _onAllConfigSubmitted = null;
                var _onEnablePeriodTransmission = null;
                var _onPeerToPeerModeSwitch = null;
                ...
                var _Self = this;
                var mSlotAmount = profileData.Dev.length;
                var mSavedP2PMode = null;
                ...
                this.setP2PModePanel = function(mdConfigJson) {
                    try{
                        ...
                        throw new Error("PeerToPeerMainConfigPanel: Setting P2P mode parameter failed." + e)
                    }
                };
                ...
                this.setP2PConfigPanel = function(commonConfigJson, idx) {
                    try{
                        ...
                        throw new Error("Setting P2P parameter failed. Error:" + e)
                    }
                };
                ...
                var toggleMd = function(md) {
                    ...
                    if(mode !== 0) {
                        ...
                        mBaseMdConfigPanel = new Advantech.Form.AdvancedForm.P2pConfigPanelFactory().instanceP2pPanel(mode, (mode === 1) ? _Self.getBasicMdPanelId() : _Self.getAdvancedMdPanelId());
                        ...
                        $("#" + _Self.getPanelId() + " #p2PSubmittedGroup #btnP2pConfigApplyAll").hide();
                    }
                };
                ...
                this.peerToPeerModeSwitch = function(x) {
                    if(!arguments.length) return _onPeerToPeerModeSwitch;
                    _onPeerToPeerModeSwitch = x;
                };
                ...
                var handleApplyEvent = function() {
                    $("#" + _Self.getPanelId() + " #btnP2pConfigApply").click(function() {
                        if($.isFunction(_Self.configSubmitted())) {
                            try{
                                //handle mode change
                                var modeJson = getModeConfigPanel().toJsonFormat();
                                if(mSavedP2PMode != modeJson.Md){
                                    Advantech.Form.MessageForm.getInstance().showMessageBox("Notification", "<i class='fa fa-fw fa-exclamation-triangle'></i>Please switch to desired Mode before Apply." , null);
                                    return;
                                }
                                if($.isFunction(_Self.peerToPeerModeSubmitted())){
                                    _Self.peerToPeerModeSubmitted()(this, modeJson);
                                }
                                if(modeJson.Md !=0)
                                    $("#" + getModeConfigPanel().getPanelId() + " #p2pCommonConfigGroup").show();
                                else{
                                    $("#" + getModeConfigPanel().getPanelId() + " #p2pCommonConfigGroup").hide();
                                    return;
                                }
                                ...
                                Advantech.Form.MessageForm.getInstance().showMessageBox("Notification","<h5>"+e+"</h5>");
                                return;
                            }
                        }
                    });
                ...
                var initialModeConfigPanel = function() {
                    ...
                    getModeConfigPanel().onPeerToPeerModeSwitch(function(obj, e) {
                        if($.isFunction(_Self.peerToPeerModeSwitch())) {
                            _Self.peerToPeerModeSwitch()(this, e);
                        }
                    });
                };

                var initialCommonConfigPanel = function() {
                    mCommonConfigPanel = new Advantech.Form.AdvancedForm.PeerToPeerCommonConfigPage(p2PCommonConfigPageId);
                ...
                this.toJson = function() {
                    ...
                    for(var i = 0; i < pageList.length; ++i) {
                        ...
                        if(pageObj["BiUn" + i])
                            ...
                            biUnArray.push(0);
                        if(pageObj["Inv" + i])
                            invertArray.push(pageObj["Inv" + i]);
                        else
                            invertArray.push(0);
                    }
                    jsonObj["ChM"] = chMArray;
                    jsonObj["BiUn"] = biUnArray;
                    jsonObj["Inv"] = invertArray;
                    return jsonObj;
                };
            },
            ...
            P2pConfigPanelFactory: function() {
                this.instanceP2pPanel = function(md, panelId) {
                    if(md === 1) {
                        return new Advantech.Form.AdvancedForm.PeerToPeerBasicMdPanel(panelId);
                    }else if(md === 2) {
                        return new Advantech.Form.AdvancedForm.PeerToPeerAdvancedMdConfigPanel(panelId);
                    }else{
            ...
            PeerToPeerBaseMdConfigPanel: function(panelId) {
                ...
                this.show = function() {
                    $("#" + this.getPanelId()).show();
                }

                this.hide = function() {
                    $("#" + this.getPanelId()).hide();
                }
            ...
            PeerToPeerCommonConfigPage: function(panelId) {
                ...
                this.initialPage = function(profileObj, md) {
                    _Self.clearPage();
                    mSlotAmount = profileObj.Dev.length;
                    ...
                    for(var i = 0; i < mSlotAmount; ++i) {
                        ...
                        var diChannelAmount = (slotProfile === null || typeof(slotProfile.DIn)=='undefined')? 0: parseInt(slotProfile.DIn);
                        var aiChannelAmount = (slotProfile === null || typeof(slotProfile.AIn)=='undefined')? 0: parseInt(slotProfile.AIn);
                        var uiChannelAmount = (slotProfile === null || typeof(slotProfile.UIn)=='undefined')? 0: parseInt(slotProfile.UIn);
                        var channelInfoJson = {"di": diChannelAmount, "ai": aiChannelAmount, "ui": uiChannelAmount};
                        mdFlag = (slotProfile !== null) ? ((aiChannelAmount > 0 || diChannelAmount > 0 || uiChannelAmount > 0) ? md : 0) : 0;
                        mPageList[i] = new Advantech.Form.AdvancedForm.
                            P2pConfigPageFactory().instanceP2pPage(mdFlag, mSlotAmount,
                                "p2pMdCofigPage" + i,
                                i, (slotProfile !== null) ? (aiChannelAmount + diChannelAmount + uiChannelAmount) : null,
                                channelInfoJson);
                        mPageList[i].initialPage(_Self.getPangeIdBySlot(i));
                    }
                    if(mSlotAmount == 1){//if only one slot, modify Tab name of Slot 0
                        $("#" + _Self.getPanelId() + " #ListSlot0").html("Configuration");
                        for(var i = mSlotAmount; i < 5; i++){ //hide other Slot's Tab
                            $("#" + _Self.getPanelId() + " #ListSlot" + i).hide();
                        }
                        $("#p2PSubmittedGroup #btnP2pConfigApplyAll").hide();//hide Apply All button
                        $("#P2pAdvConfigModal .SrcCouplerType").hide();//hide src slot
                        $("#P2pAdvConfigModal .DesCouplerType").hide();//hide dst slot
                        $("#p2PCommonConfigCotainer .displayInMultiSlot").hide();
                    }else{
                        $("#p2PSubmittedGroup #btnP2pConfigApplyAll").show();//show Apply All button
                    }
                    mP2pToJson = new Advantech.Form.AdvancedForm.P2pConfigToJsonFactory().instanceP2pToJson(md, mPageList);
                }
                ...
                this.setP2pConfigPanel = function(jsonObj, index) {
                    try{
                        $('#' + _Self.getPanelId() + " #advConfigJson").val(JSON.stringify(jsonObj));//store json
            ...
            PeerToPeerBasicMdPanel: function(panelId) {
                ...
                this.toP2pConfigJson = function() {
                    ...
                    obj.PItv = parseInt($("#" + this.getPanelId() + " #inpP2pBasicMdPeriodTime").val(), 10);
                    obj.COS = parseInt(($("#" + this.getPanelId() + " #inpP2pBasicMdEnCos").prop('checked') === true) ? 1 : 0, 10);
                    var dev = parseInt($("#" + this.getPanelId() + " #inpP2pBasicMdDevVal").val(), 10);
                    if(!$.isNumeric(dev) || dev < 1 || dev >100)
                        throw new Error("Deviation Value must between 1~100");
                    obj.Dev = dev;
                    obj.Id = parseInt($("#" + this.getPanelId() + " #inpP2pBasicMdDestModel").val(), 10);
                    obj.Pw = $("#" + this.getPanelId() + " #inpP2pBasicMdDestModelPw").val();
                    obj.Pw = Advantech.Utility.getPasswordEncoding("", obj.Pw);
                    return obj;
                };
                ...
                this.setP2pConfigPanel = function(jsonObj) {
                    try{
                        ...
                        $("#" + this.getPanelId() + " #inpP2pBasicMdPeriodTime").val(jsonObj.PItv);
                        ...
                        $("#" + this.getPanelId() + " #inpP2pBasicMdDestModel").val(jsonObj.Id);
                        $("#" + this.getPanelId() + " #inpP2pBasicMdDestModelPw").val(Advantech.Utility.getPasswordDecoding(jsonObj.Pw).trim());
                    }catch(e){
            ...
            PeerToPeerMdConfigBasePage: function(slotAmount, pageId, slotIdx, chNum, channelInfoJson) {
                var mContextPageId = pageId;
                var mChannelNumber = chNum;
                //var mIsAIMode = isAIMd;
                var mChannelInfoJson = channelInfoJson;
                ...
                this.getIsAIType = function() {
                    //return mIsAIMode;
                    return mChannelInfoJson.ai > 0 ? true : false;
                };
                this.getChannelInfo = function() {
                    return mChannelInfoJson;
                };
                this.getChannelNumber = function() {
                    return mChannelNumber;
                };
            ...
            PeerToPeerNotSupportConfigPage: function(pageId, slotIdx, chNum, channelInfoJson) {
            ...
            /*Inheritance from PeerToPeerMdConfigBasePage*/
            PeerToPeerBasicMdConfigPage: function(pageId, slotIdx, chNum, channelInfoJson) {
                Advantech.Form.AdvancedForm.PeerToPeerMdConfigBasePage.apply(this, arguments);
                var mSlotIndex = slotIdx.slice(14);
                var _Self = this;
                this.initialPage = function(containerName) {
                    var p2pBasicMdConfigContainer = document.createElement('div');
                    p2pBasicMdConfigContainer.className = 'col-lg-12';
                    ...
                    p2pBasicMdConfigContainer.innerHTML = "<div class = 'row'>" +
                        ...
                        "<th>Channel</th>" +
                        "<th>Enable</th>" +
                        "<th class = 'P2pAiType'>Only Positive Value Valid</th>" +
                        "<th>Invert Signal</th>" +
                        "</tr>" +
                        "</thead>" +
                        "<tbody>" +
                ...
                var intialConfigTable = function() {
                    ...
                    $("#" + _Self.getPageId() + " #checkboxChP2pInvertEnableAll").change(function() {
                        $("#" + _Self.getPageId() + " input[name=ChP2pInvertEnable]").prop('checked', this.checked);
                    });
...
/*                    if(!_Self.getIsAIType()) {
                ...
                var addRow = function(index) {
                    ...
                    for (var i = 0; i < 4; ++i) {
                        ...
                        if(i == 0) {
                            if(index != 'All')
                                cell.innerHTML = "<span id='ChP2pChannel"+index + "'></span>";
                            else
                                cell.innerHTML = index;
                        }else if(i == 1) {
                            ...
                            cell.innerHTML = "<input type='checkbox' name='ChP2pInvertEnable' id='checkboxChP2pInvertEnable" + index + "'>";
                        }
                    }
                ...
                this.toP2pConfigJson = function() {
                    ...
                    var enableInvert = [];
                    $("#" + _Self.getPageId() + " input[name=ChP2pInvertEnable]").each(function() {
                        var idx = this.id.slice(25);
                        if(idx !== 'All') {
                            enableInvert[idx] = this.checked;
                        }
                    });
                    p2PObj['Inv' + _Self.getSlotIndex()] = Advantech.Utility.convertArrayToMask(enableInvert);
                    return p2PObj;
                };

                this.setP2pConfigPage = function(jsonObj) {
                    try{
                        var nameArray = Advantech.Utility.convertMaskToArray(parseInt(jsonObj.TyM[mSlotIndex], 10), this.getChannelNumber());
                        var diCount =0;
                        var aiCount =0;
                        for(var i = 0; i < this.getChannelNumber(); ++i) {
                            var channelName = '';
                            if(nameArray[i] == 1){
                                channelName = 'AI_' + aiCount;
                                aiCount++;
                            }else{
                                channelName = 'DI_' + diCount;
                                diCount++;
                            }
                            $("#" + _Self.getPageId() + " #ChP2pChannel" + i).html(channelName);
                        }
                        var chArray = Advantech.Utility.convertMaskToArray(parseInt(jsonObj.ChM[mSlotIndex], 10), this.getChannelNumber());
                        for(var i = 0; i < this.getChannelNumber(); ++i) {
                            $("#" + _Self.getPageId() + " #checkboxChP2pRunEnable" + i).prop('checked', chArray[i]);
                        }
                        var invArray = Advantech.Utility.convertMaskToArray(parseInt(jsonObj.Inv[mSlotIndex], 10), this.getChannelNumber());
                        for(var i = 0; i < this.getChannelNumber(); ++i) {
                            $("#" + _Self.getPageId() + " #checkboxChP2pInvertEnable" + i).prop('checked', invArray[i]);
                        }
                        //if(_Self.getIsAIType()){
                            var BiUnArray = Advantech.Utility.convertMaskToArray(parseInt(jsonObj.BiUn[mSlotIndex], 10), this.getChannelNumber());
                            for(var i = 0; i < this.getChannelNumber(); ++i) {
                                if(nameArray[i] == 1) //AI
                                    $("#" + _Self.getPageId() + " #checkboxChP2pBiToUniEnable" + i).prop('checked', BiUnArray[i]);
                                else{
                                    $("#" + _Self.getPageId() + " #checkboxChP2pBiToUniEnable" + i).prop('checked', 0);
                                    $("#" + _Self.getPageId() + " #checkboxChP2pBiToUniEnable" + i).hide();
                                }
                            }
                        //}
                    }catch(e){
            ...
            /*Inheritance from PeerToPeerMdConfigBasePage*/
            PeerToPeerAdvancedMdConfigPage: function(slotAmount, pageId, slotIdx, chNum, channelInfoJson) {
                Advantech.Form.AdvancedForm.PeerToPeerMdConfigBasePage.apply(this, arguments);
                var INVALID = '****';
                var _Self = this;
                var mSlotAmount = slotAmount;
                var mChNumber = chNum;
                ...
                this.initialPage = function(containerName) {
                    ...
                    p2pAdvMdConfigContainer.innerHTML = "<div class = 'row'>" +
                        ...
                        "<th>Map to IP</th>" +
                        "<th>Map to Module</th>" +
                        "<th class='displayInMultiSlot'>Map to Slot</th>" +
                        "<th>Map to ch</th>" +
                        "<th class = 'P2pAiType'>Only Positive Value Valid</th>" +
                ...
                var addRow = function(index) {
                    ...
                    for (var i = 0; i < 14; ++i) {
                        ...
                        if(i == 0) {
                            ...
                            cell.innerHTML = INVALID;
                            cell.className = 'displayInMultiSlot';
                        }else if(i == 10) {
                            cell.innerHTML = INVALID;
                        }else if(i == 11) {
                            cell.innerHTML = INVALID;
                    ...
                    $("#" + _Self.getPageId() + " #btnP2PAdvConfig" + index).click(function() {
                        var configModal = new Advantech.Form.AdvancedForm.P2pAdvancedConfigModal('P2pAdvConfigModal', _Self.getChannelInfo(), mSlotAmount>1?true:false, index, _Self.getSlotIndex());
                        configModal.setP2pModalConfig(index, _Self.toP2pConfigJson().Adv[index]);
                        configModal.show();
                    ...
                    try{
                        $("#" + _Self.getPageId() + " tbody #" + pageId + "Row" + idx).each(function() {
                            if(this.cells){
                                var channelName = '';
                                if(obj.Ty == 1){
                                    channelName = 'AI_' + obj.TyCh;
                                    (parseInt(obj.BiUn, 10) === 1) ? $(this.cells[11]).text("true") : $(this.cells[11]).text("false");
                                    $(this.cells[12]).text(obj.Dev);
                                }
                                else{
                                    channelName = 'DI_' + obj.TyCh;
                                    $(this.cells[11]).text(INVALID);
                                    $(this.cells[12]).text(INVALID);
                                }
                                $(this.cells[1]).text(channelName);

                                (parseInt(obj.En, 10) === 1) ? $(this.cells[2]).text("true"): $(this.cells[2]).text("false");
                                $(this.cells[3]).text(Advantech.Profile.P2PChannelInputMode[obj.InM]);
                                $(this.cells[4]).text(Advantech.Profile.P2PChannelOutputMode[obj.OutM]);
                                (parseInt(obj.COS, 10) === 1) ? $(this.cells[5]).text('Yes'): $(this.cells[5]).text('No');
                                $(this.cells[6]).text(obj.PItv);
                                $(this.cells[7]).text(obj.IP);
                                $(this.cells[8]).text(Advantech.Profile.P2PdestinationModuleName[obj.Id]);
                                $(this.cells[9]).text(obj.SL || INVALID);
                                $(this.cells[10]).text(obj.DCh);
/*                                $(this.cells[11]).text(INVALID);
                                $(this.cells[12]).text(INVALID);
                                if(_Self.getIsAIType())
                                    (parseInt(obj.BiUn, 10) === 1) ? $(this.cells[11]).text("true") : $(this.cells[11]).text("false");
                        ...
                        var advJson = JSON.parse($("#p2PCommonConfigCotainer #advConfigJson").val());
                        if(advJson['Adv'][idx]){
                            advJson['Adv'][idx] = obj;
                            $("#p2PCommonConfigCotainer #advConfigJson").val(JSON.stringify(advJson));//store json
                        }
                    }catch(e){
                ...
                this.getP2pConfigRowJson = function(idx) {
                    var slotIdx = pageId.slice(14); //pageId ex: p2pMdCofigPag0
                    var advJson = JSON.parse($("#p2PCommonConfigCotainer #advConfigJson").val());
                    var jsonObj = advJson['Adv'][idx];
            ...
            P2pAdvancedConfigModal: function(modalId, channelInfoJson, isCoupler, srcChannel, srcSlot) {
                ...
                this.setP2pModalConfig = function(idx, obj) {
                    try{
                        var channelName = '';
                        if(obj.Ty == 1){
                            channelName = 'AI_' + obj.TyCh;
                            $("#" + _Self.getModalId() + " .AIType").show();
                        }
                        else{
                            channelName = 'DI_' + obj.TyCh;
                            $("#" + _Self.getModalId() + " .AIType").hide();
                        }
                        $("#" + _Self.getModalId() + " #txtbarChName").val(channelName);
                        $("#" + _Self.getModalId() + " #txtbarChType").val(obj.Ty);
                        $("#" + _Self.getModalId() + " #txtbarChNumber").val(obj.TyCh);
                        ...
                        $("#" + _Self.getModalId() + " #txtbarPeriodTime").val(obj.PItv);
                        ...
                        $("#" + _Self.getModalId() + " #selp2PAdvOutMode").val(obj.OutM);
                        $("#" + _Self.getModalId() + " #selp2PAdvInpMode").val(obj.InM).trigger('change');
                        $("#" + _Self.getModalId() + " #txtbarAOH").val(obj.AOH);
                        $("#" + _Self.getModalId() + " #txtbarAOL").val(obj.AOL);
                        $("#" + _Self.getModalId() + " #txtbarp2PAdvDesPwd").val(Advantech.Utility.getPasswordDecoding(obj.Pw).trim());
                        $("#" + _Self.getModalId() + " #inpP2pAdvDevVal").val(obj.Dev);
                        $("#" + _Self.getModalId() + " #inpP2pInv").prop('checked', (parseInt(obj.Inv, 10) === 1) ? true : false);
                ...
                this.toP2pConfigJson = function() {
                    var pwd = $("#" + _Self.getModalId() + " #txtbarp2PAdvDesPwd").val();
                    pwd = Advantech.Utility.getPasswordEncoding("", pwd);
                    var id = $("#" + _Self.getModalId() + " #selp2PAdvModule").val();
                    if(!id) id = 65535;
                    var jsonObj = {
                        Ch: Number($("#" + _Self.getModalId() + " #txtbarCh").val()),
                        Ty: Number($("#" + _Self.getModalId() + " #txtbarChType").val()),
                        TyCh: Number($("#" + _Self.getModalId() + " #txtbarChNumber").val()),
                        En: ($("#" + _Self.getModalId() + " #inpP2pisEnable").prop('checked') === true) ? 1 : 0,
                        COS: ($("#" + _Self.getModalId() + " #inpP2pCos").prop('checked') === true) ? 1 : 0,
                        Inv: ($("#" + _Self.getModalId() + " #inpP2pInv").prop('checked') === true) ? 1 : 0,
                        PItv: Number($("#" + _Self.getModalId() + " #txtbarPeriodTime").val()),
                        IP: $("#" + _Self.getModalId() + " #txtbarp2PAdvDesIp").val(),
                        Id: Number(id),
                        DCh: Number($("#" + _Self.getModalId() + " #selp2PAdvChannel").val()),
                        InM: Number($("#" + _Self.getModalId() + " #selp2PAdvInpMode").val()),
                        OutM: Number($("#" + _Self.getModalId() + " #selp2PAdvOutMode").val()),
                        Pw: pwd,
                        AOH: Number($("#" + _Self.getModalId() + " #txtbarAOH").val()),
                        AOL: Number($("#" + _Self.getModalId() + " #txtbarAOL").val()),
                    };
                    //if(_Self.getIsAiType()) {
                        var dev = parseInt($("#" + _Self.getModalId() + " #inpP2pAdvDevVal").val(), 10);
                        if(!$.isNumeric(dev) || dev < 1 || dev >100)
                            throw new Error("Deviation Value must between 1~100");
                        jsonObj["Dev"] = dev;
...
/*                    if(!_Self.getIsAiType()) {
                    ...
                    if(!_Self.getIsCoupler()) {
                        $("#" + _Self.getModalId() + " .SrcCouplerType").hide();
                    }else{
                        $("#" + _Self.getModalId() + " .SrcCouplerType").show();
                    }
                    ...
                    for (var product in DeviceEmun) {
                        if(DeviceEmun.hasOwnProperty(product)) {
                            if(DeviceEmun[product].isSupportP2P) {
                                if(_Self.getChannelInfo().di > 0 || _Self.getChannelInfo().ai > 0 || _Self.getChannelInfo().ui > 0){ //DI, AI can map to DO, AO
                                    if(DeviceEmun[product].do_Ch > 0 || DeviceEmun[product].ao_Ch > 0)
                                        $("#" + _Self.getModalId() + " #selp2PAdvModule").append("<option value='" + Advantech.Profile.P2PdestinationModuleID[product] + "'>" + product + "</option>");
                                }
                            }
                        }
                    }

                    $("#" + _Self.getModalId() + " #selp2PAdvInpMode").change(function(){
                        var channelType = $("#" + _Self.getModalId() + " #txtbarChType").val();
                        var channelInputMd = $("#" + _Self.getModalId() + " #selp2PAdvInpMode").val();
                        if(channelType == 1 && channelInputMd==0 ){
                            $("#" + _Self.getModalId() + " #selp2PAdvOutMode option[value=0]").prop('disabled', true);
                            $("#" + _Self.getModalId() + " #selp2PAdvOutMode").val('1');
                        }else
                            $("#" + _Self.getModalId() + " #selp2PAdvOutMode option[value=0]").prop('disabled', false);
                    });

                    $("#" + _Self.getModalId() + " #selp2PAdvModule").change(function() {
                        var module = $(this).val();
                        module = Advantech.Profile.P2PdestinationModuleName[module];
                        if(module) {
                            if(DeviceEmun[module].isCoupler === true) {
                                $("#" + _Self.getModalId() + " .DesCouplerType").show();
                            }else{
                                $("#" + _Self.getModalId() + " .DesCouplerType").hide();
                            }

                            $("#" + _Self.getModalId() + " #selp2PAdvChannel").find('option').remove();
                            var channelNum = 0;
                            channelNum = parseInt(DeviceEmun[module].ao_Ch + DeviceEmun[module].do_Ch);
                    ...
                    $("#" + _Self.getModalId() + " #btnAdvConfig").click(function() {
                        if($.isFunction(_Self.onSubmitted()))
                            try{
                                ...
                                Advantech.Form.MessageForm.getInstance().showMessageBox("<h4>Format Error<h4>", "<h5>" + e + "</h5>");
                                return;
                            }
                    });
