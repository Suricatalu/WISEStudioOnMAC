...
self.addEventListener('message', function(e) {
	...
	if(cmd.command == 'parseJson'){
		self.postMessage({command:cmd.command, data:json2Array(cmd.data, cmd.isWISE2411)});
	}
...
var json2Array = function(jsonObj, isWISE2411){
    ...
    for (var i = 0; i < logMsgLen; ++i) {
		if (isWISE2411) {
			...
			var rms = ["-","-","-","-","-","-","-","-","-"];
			var temperature = "-";
			//var rowData = [logMsg[i].Id, logMsg[i].PE, (logMsg[i].TIM || "****")];
			var rowData = [logMsg[i].Id, logMsg[i].PE, logMsg[i].TIM];
		} else {
			var pe = logMsg[i].PE;
        ...
        for (var j = 0; j < recordArray.length; ++j) {
			if (isWISE2411) {
				switch (recordArray[j][1]) {
					case 0:
						switch (recordArray[j][2]) {
							case 45:
								vibration[0] = recordArray[j][3];
								break;
							case 46:
								vibration[1] = recordArray[j][3];
								break;
							case 47:
								vibration[2] = recordArray[j][3];
								break;
							case 48:
								vibration[3] = recordArray[j][3];
								break;
							case 49:
								vibration[4] = recordArray[j][3];
								break;
							case 50:
								vibration[5] = recordArray[j][3];
								break;
							case 51:
								vibration[6] = recordArray[j][3];
								break;
							case 52:
								vibration[7] = recordArray[j][3];
								break;
							case 53:
								vibration[8] = recordArray[j][3];
								break;
							case 54:
								vibration[9] = recordArray[j][3];
								break;
							case 55:
								vibration[10] = recordArray[j][3];
								break;
							case 110:
								rms[0] = recordArray[j][3];
								break;
							case 111:
								rms[1] = recordArray[j][3];
								break;
							case 112:
								rms[2] = recordArray[j][3];
								break;
							case 113:
								rms[3] = recordArray[j][3];
								break;
							case 114:
								rms[4] = recordArray[j][3];
								break;
							case 115:
								rms[5] = recordArray[j][3];
								break;
							case 116:
								rms[6] = recordArray[j][3];
								break;
							case 117:
								rms[7] = recordArray[j][3];
								break;
							case 118:
								rms[8] = recordArray[j][3];
								break;
							case 119:
								rms[9] = recordArray[j][3];
								break;
						}
						break;
					case 1:
		...
		if (isWISE2411) {
			rowData.push(vibration.toString(), rms.toString(), temperature)
			rawDataArray.push(rowData);
		}
    }
    var slotArray = [];
